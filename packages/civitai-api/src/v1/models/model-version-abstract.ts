 ...
 */
export interface ModelsVersion {
  id: number;
  name: string;
  baseModel: string;
  baseModelType: string | null;
 ...
 */
export interface ModelByIdVersion {
  id: number;
  name: string;
  baseModel: string;
  baseModelType: string | null;
 ...
 */
export interface ModelVersionEndpoint {
  id: number;
  modelId: number;
  name: string;
  baseModel: string;
 ...
 */
export interface ModelsEndpointModel {
  id: number;
  name: string;
  description: string | null;
  type: ModelTypes;
...
export interface ModelByIdEndpointModel {
  ...
  stats: ModelStats;
  tags: string[];
  modelVersions: ModelByIdVersion[];
}
 ...
 */
export type ModelVersionAny = ModelsVersion | ModelByIdVersion | ModelVersionEndpoint;

/**
 * Union type of all possible Model types from different endpoints
 * Use this when you need to accept any Model type
 */
export type ModelAny = ModelsEndpointModel | ModelByIdEndpointModel;

/**
 * Type guard to check if a value is from the models endpoint
 */
export function isModelsVersion(version: ModelVersionAny): version is ModelsVersion {
 ...
 */
export function isModelByIdVersion(version: ModelVersionAny): version is ModelByIdVersion {
  const v = version as any;
  return v.index !== undefined && v.availability !== undefined;
}
 ...
 */
export function isModelVersionEndpoint(version: ModelVersionAny): version is ModelVersionEndpoint {
  const v = version as any;
 ...
 * Type guard to check if a value is from the models endpoint
 */
export function isModelsEndpointModel(model: ModelAny): model is ModelsEndpointModel {
...
export function getIndex(version: ModelVersionAny): number | undefined {
  if (isModelsVersion(version) || isModelByIdVersion(version)) {
    return (version as ModelsVersion | ModelByIdVersion).index;
  }
  return undefined;
}
...
export function getAvailability(version: ModelVersionAny): 'EarlyAccess' | 'Public' | undefined {
  if (isModelsVersion(version) || isModelByIdVersion(version)) {
    return (version as ModelsVersion | ModelByIdVersion).availability;
  }
  return undefined;
}
...
export function getPublishedAt(version: ModelVersionAny): string | null | undefined {
  if (isModelsVersion(version)) {
    const v = version as ModelsVersion;
  ...
  if (isModelByIdVersion(version)) {
    const v = version as ModelByIdVersion;
    return v.publishedAt; // string | undefined
  }
  if (isModelVersionEndpoint(version)) {
    const v = version as ModelVersionEndpoint;
 ...
 */
export function getImageId(image: ModelImage): Result<number, Error> {
 ...
 */
export function ensureImageIds(images: ModelImage[]): Result<ModelImage[], Error> {
