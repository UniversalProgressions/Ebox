...
describe("ModelVersionAbstract", () => {
  ...
  describe("Utility Functions", () => {
    ...
    test("getPublishedAt handles different nullability across endpoints", () => {
      expect(getPublishedAt(mockModelsVersion)).toBe("2024-01-01T00:00:00.000Z");
      expect(getPublishedAt(mockModelByIdVersion)).toBe("2024-02-01T00:00:00.000Z");
      expect(getPublishedAt(mockModelVersionEndpoint)).toBe("2024-03-01T00:00:00.000Z");
    });
  });

  describe("findModelVersion", () => {
    const versions = [mockModelsVersion, mockModelByIdVersion, mockModelVersionEndpoint];
...
describe("extractIdFromImageUrl", () => {
  ...
  test("extracts ID from URL with query parameters", () => {
    const url = "https://image.civitai.com/xG1nkqKTMzGDvpLrqFT7WA/cbe20dcf-7721-4f34-bc24-9ff14b96cab2/width=1024/6039981.jpeg?token=abc123";
    const result = extractIdFromImageUrl(url);
    expect(result.isOk()).toBe(true);
  ...
  test("extracts ID from URL without file extension", () => {
    const url = "https://image.civitai.com/xG1nkqKTMzGDvpLrqFT7WA/cbe20dcf-7721-4f34-bc24-9ff14b96cab2/width=1024/123456";
    const result = extractIdFromImageUrl(url);
    expect(result.isOk()).toBe(true);
...
describe("Image ID Extraction", () => {
  ...
  test("ensureImageIds processes array of images with mixed IDs", () => {
    ...
    if (result.isOk()) {
      ...
      expect(processedImages[1]?.url).toBe("https://image.civitai.com/xG1nkqKTMzGDvpLrqFT7WA/cbe20dcf-7721-4f34-bc24-9ff14b96cab2/width=1024/222222.jpeg");
