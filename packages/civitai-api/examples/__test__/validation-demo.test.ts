...
describe('Arktype Validation', () => {
  ...
  it('should demonstrate error type checking', () => {
    ...
    const mockValidationError = {
      type: 'validation',
      message: 'Validation failed: Expected number at \'items[0].id\', got string (\'not-a-number\')',
      details: {
    ...
    if (hasValidationStructure) {
      ...
      expect(mockValidationError.details.problems[0]!.message).toContain('Expected number');
    }
  });
});