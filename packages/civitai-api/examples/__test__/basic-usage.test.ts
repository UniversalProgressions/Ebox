...
describe('Civitai API Client Basic Usage', () => {
  ...
  it('should fetch creators list', async () => {
    ...
    if (creatorsResult.isOk()) {
      ...
      if (creators.metadata.totalItems !== undefined && creators.metadata.totalItems !== null) {
  ...
  it('should fetch models list', async () => {
    ...
    if (modelsResult.isOk()) {
      ...
      if (models.metadata.totalItems !== undefined && models.metadata.totalItems !== null) {
  ...
  it('should fetch tags list', async () => {
    ...
    if (tagsResult.isOk()) {
      ...
      // totalItems might be undefined or null
      if (tags.metadata.totalItems !== undefined && tags.metadata.totalItems !== null) {
  ...
  it('should demonstrate unified type system', async () => {
    ...
    if (unifiedModelsResult.isOk() && unifiedModelByIdResult.isOk()) {
      ...
      if (unifiedModelsResult.isErr()) {
        console.log(`Unified models list API error: ${unifiedModelsResult.error.message}`);
      }
      if (unifiedModelByIdResult.isErr()) {
        console.log(`Unified model by ID API error: ${unifiedModelByIdResult.error.message}`);
      }
    }
  }, 15000);

  it('should use unified client endpoints', async () => {
    // Using unified model endpoints
    ...
    if (unifiedResult.isOk()) {
      ...
      expect(unifiedModel).toBeDefined();
      expect(unifiedModel.name).toBeDefined();
      expect(unifiedModel.type).toBeDefined();
      expect(unifiedModel.modelVersions).toBeDefined();
      expect(Array.isArray(unifiedModel.modelVersions)).toBe(true);
      ...
      if (coreResult.isOk()) {
        ...
        console.log(`Core fields API error: ${coreResult.error.message}`);
      }
    } else {
      console.log(`Unified model API error: ${unifiedResult.error.message}`);
    }
  }, 10000);
});